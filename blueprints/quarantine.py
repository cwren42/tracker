    ...
    for (sdom, reason), agg in by_key.items():
        ...
        if kind == "transport_rule":
            ...
            test_ps = (
                ...
                f'# A transport rule can block by SENDER (SenderDomainIs/From) OR by CONTENT\n'
                f'# (subject/body/attachments) — show what THIS rule actually matches on:\n'
                f'$r | Format-List Name,State,SenderDomainIs,From,FromAddressContainsWords,SubjectContainsWords,SubjectOrBodyContainsWords,AttachmentExtensionMatchesWords,AttachmentContainsWords,MessageSizeOver\n'
                f'"Current sender exceptions (ExceptIfSenderDomainIs): $($r.ExceptIfSenderDomainIs -join \', \')"\n'
                f'# The robust unblock for one sender is an EXCEPTION (works whether the rule is\n'
                f'# sender- or content-based) — scoped to this sender, the rule still applies to everyone else.\n'
                f'$already = @($r.ExceptIfSenderDomainIs | Where-Object {{ $sender -eq $_ -or $sender.EndsWith("." + $_) }})\n'
                f'if ($already) {{ "ALREADY EXEMPT via [$($already -join \', \')] — \'$rule\' should not block \'$sender\'. (If it still is, a DIFFERENT rule is to blame — re-run message trace.)" }} else {{ "VALIDATION OK — \'$sender\' is not yet exempt. LIVE will add it to ExceptIfSenderDomainIs to unblock it from \'$rule\'." }}'
            )
            live_ps = (
                lhdr +
                f'$rule   = "{rule}"\n'
                f'$sender = "{safe}"\n'
                f'$r = Get-TransportRule $rule\n'
                f'# Exempt this sender domain from the rule (scoped — everyone else stays subject to it).\n'
                ...
                f'$now = @((Get-TransportRule $rule).ExceptIfSenderDomainIs)\n'
                f'if ($now -contains $sender) {{ "UNBLOCKED — \'$sender\' is now exempt from \'$rule\'. Exceptions: $($now -join \', \')" }} else {{ "CHECK — exception not applied; review the rule manually." }}'
            )
